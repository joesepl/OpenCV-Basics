...
def translate(img, x, y):
    ...
    print(transMat)
    dimensions = (img.shape[1], img.shape[0])   #this gets the height,width (page,row) of the picture. It reverses the normal order of hieght, width to width, height. It doesn't do anything with the columms which are the channels. 
...
def rotate(img, angle, rotPoint=None):
    (height, width) = img.shape[:2]     #shape returns a dimension tuple like (100,100,3). For us we only care about the height and width of the image so we only get 100, and 100 of the tuple

    if rotPoint is None:
        rotPoint = (width//2, height//2)        # set rotation point to the midway point of width and height
    ...
    rotMat = cv.getRotationMatrix2D(rotPoint, angle, 1.0) #parameters: roation point, angle in radians, and scale. 
...
#Flipping
flip = cv.flip(img, -1) #flip code can be 0, 1, or -1. Flip image verticallly (0), flip it horizontally (1), flip it horizontally and vertically (-1)
cv.imshow('flip', flip)
