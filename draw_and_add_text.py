...
blank = np.zeros((500,500,3), dtype='uint8')   #create a blank 500 by 500 image of data type uint8. It has 3 channels (columns) for bgr except its ordered bgr.
print(blank)
...
cv.rectangle(blank, (0,0), (250,250), (0,255,0), thickness=cv.FILLED) #cv.rectangle(image_array, start_coord, end_coord, bgr_color, thickness=2). Filled keyword can also be -1. This will fill the whole rectangle.
...
radius = int(blank.shape[0] / 2)    #find radius to fill full page by dividing width by 2.
...
#4. Draw Line
cv.line(blank, (blank.shape[0],blank.shape[1]), (0,0), (255,255,0), thickness=2 )       #line(image_array, (start_coord_x, start_coord_y), (end_coord_x, end_coord_y), (b,g,r), thickness=2)


#5. Write text
cv.putText(blank, 'Hello', (0, 225), cv.FONT_HERSHEY_TRIPLEX, 1.0, (255,0,0), thickness=3) #putText(img, text, coord, fontFace, fontscale, color, thickenss=None)
